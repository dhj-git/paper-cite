...
`POST /api/pdf-batches` 接收最多 50 个文件并立即返回任务 ID，后台使用 3 个并发槽处理；前端通过 `GET /api/pdf-batches/{batch_id}` 每秒轮询进度。单文件限制为 15 MB，整批限制为 250 MB。同一批次中相同 DOI 或规范化标题共享一次检索请求。
...
- 批量上传会暂存在内存中直到任务完成；正式部署处理大量文件时应改用临时文件或对象存储。
